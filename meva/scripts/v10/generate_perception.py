...
def generate_perception_qa(sg: SceneGraph, resolved: ResolvedGraph,
    ...
    for act, correct_cams in type1_pool:
        ...
        options = [f"Camera {correct_cam}"] + [f"Camera {d}" for d in distractors[:3]]
        rng.shuffle(options)
    ...
    if len(qa_pairs) < count:
        ...
        for act, correct_cams in multi_cam_acts:
            ...
            if len(sorted_cams) >= 2:
                option_both = f"Both Camera {sorted_cams[0]} and Camera {sorted_cams[1]}"
                option_a_only = f"Camera {sorted_cams[0]} only"
                option_b_only = f"Camera {sorted_cams[1]} only"
