...
# Module-level cache for MEVID parsing (avoids re-reading files on every call)
_MEVID_PERSON_CAMERAS_CACHE: Optional[Dict[int, Set[str]]] = None
...
def parse_mevid_person_cameras() -> Dict[int, Set[str]]:
    ...
    """
    global _MEVID_PERSON_CAMERAS_CACHE
    if _MEVID_PERSON_CAMERAS_CACHE is not None:
        return _MEVID_PERSON_CAMERAS_CACHE
    ...
    _MEVID_PERSON_CAMERAS_CACHE = dict(person_cameras)
    return _MEVID_PERSON_CAMERAS_CACHE
