...
def stream_geom_records(path: Path) -> Generator[Dict, None, None]:
    ...
    with open(path) as f:
        for line in f:
            ...
            # Try inline format first (most common in Kitware geom.yml)
...
def get_actor_keyframe_bboxes(path: Path, actor_ids: Optional[Set[int]] = None,
    ...
    """
    Extract keyframe bounding boxes for specific actors (or all actors).
    
    Args:
        path: Path to geom.yml
        actor_ids: Set of actor IDs to extract (None = all)
        sample_every: Sample every Nth frame (default 30)
