...
def generate_scene_summary_qa(sg: SceneGraph, resolved: ResolvedGraph,
    ...
    if top_3:
        top_act = humanize_activity(top_3[0][0])
        correct_desc = (
            f"A {scene_type} scene across {n_cams} cameras, "
            f"primarily featuring {top_act} activity"
        )
    else:
        correct_desc = f"A {scene_type} scene across {n_cams} cameras"
    ...
    wrong_descs.append(
        f"An empty scene with minimal activity, captured on {max(1, n_cams - 3)} cameras"
    )
    wrong_descs.append(
    ...
    question = (
        f"Considering all {n_cams} camera feeds in this slot, "
