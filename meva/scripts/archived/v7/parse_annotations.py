...
from .config import MEVA_ROOT, SLOT_INDEX_PATH  # noqa: E402
...
def find_clips_for_slot(slot: str) -> List[Dict]:
    ...
    # Parse date/time/site from slot name: "2018-03-11.11-25-00.school"
    ...
    for source_name, source_dir in source_dirs.items():
        ...
        for cam_id in info["cameras"]:
            ...
            # Find matching activities.yml
            pattern = f"{date}.{slot_time}*{cam_id}*.activities.yml"
            matches = list(ann_dir.glob(pattern))
            if not matches:
                # Try minute-level match
                pattern_alt = f"{date}.{slot_time[:5]}*{cam_id}*.activities.yml"
                matches = list(ann_dir.glob(pattern_alt))
            if matches:
                act_file = matches[0]
