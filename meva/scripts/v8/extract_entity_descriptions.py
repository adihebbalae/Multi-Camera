...
def find_slot_files(slot: str) -> List[Dict]:
    ...
    Returns list of {camera, geom_path, video_path, act_path, video_format}.
    """
    # Parse slot: "2018-03-11.11-25-00.school"
    parts = slot.split(".")
    if len(parts) < 3:
    ...
    hour = time_part.split("-")[0]  # e.g. "11" from "11-25-00"

    kitware_dir = KITWARE_BASE / date / hour
    avi_dir = AVI_BASE / date / hour
    mp4_dir = MP4_BASE / date / hour

    if not kitware_dir.is_dir():
        raise FileNotFoundError(f"Kitware dir not found: {kitware_dir}")
...
def main():
    ...
    parser.add_argument("--slot", "-s", required=True,
        help="Slot to process (e.g., 2018-03-11.11-25-00.school)")
