...
class ResolvedGraph:
    ...
    mevid_person_cameras: Dict[int, Set[str]] = None  # V7: person_id -> {camera_ids}
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "mevid_person_cameras": {
                str(k): sorted(v) for k, v in (self.mevid_person_cameras or {}).items()
            },
        }
...
def resolve_entities(sg: SceneGraph, verbose: bool = False) -> ResolvedGraph:
    ...
    return ResolvedGraph(
        ...
        mevid_person_cameras=mevid_persons,
    )
