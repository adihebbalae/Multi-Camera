...
class VLMAnnotator:
    ...
    def _select_best_camera(
        ...
        Args:
            obj: Object to select camera for
            frame: Frame data
            camera_preference: Ordered list of preferred cameras
            
        Returns:
            Camera name with best visibility, or None if not visible
        """
        if not obj.visible_cameras:
            return None
        
        # If camera preference is specified, use it
        if camera_preference:
            for cam in camera_preference:
                if cam in obj.visible_cameras:
                    return cam
        
        # Otherwise, return the first visible camera
        return obj.visible_cameras[0]
    ...
    def annotate_object_combined(
        ...
        """
        Generate both activity and description for an object in one API call.
        ...
        # Construct combined prompt for activity and description
...
Please provide two things about this {class_name}:

1. ACTIVITY: What is this {class_name} doing? (1 brief sentence)
...
ACTIVITY: [your activity description]
DESCRIPTION: [your detailed description]"""
        ...
        try:
            ...
            print(f"Error getting combined annotation: {e}")
            return "unknown", "No description available"
    
    def annotate_object_caption(
        self,
        image: np.ndarray,
        obj: ObjectProperties,
        camera: str
        ...
        """
        Generate short caption for an object.
        ...
        # Construct prompt for caption
        prompt = f"""This is a view from {camera} of an autonomous vehicle.

Focus on the {class_name} at position {obj.position[0]:.1f}m forward, {obj.position[1]:.1f}m lateral.

Provide a very short caption for this {class_name} (3-5 words).
Examples: "blue car turning left", "pedestrian crossing street", "stopped red truck"."""
    ...
    def _annotate_single_object(
        ...
        """
        Annotate a single object (to be run in parallel).
        
        Args:
            obj: Object to annotate
            image: Camera image
            camera: Camera name
            context_objects: Other objects visible in same camera
            frame: Frame data
            
        Returns:
            Annotated object dictionary or None if error
        """
        try:
            # Get combined activity and description
            activity, description = self.annotate_object_combined(
                image, obj, context_objects, camera
            )
            
            # Get caption
            caption = self.annotate_object_caption(
                image, obj, camera
            )
    ...
    def annotate_frame_objects(
        ...
        """
        Annotate all objects in a frame using camera images (parallelized).
        ...
        # Group objects by camera (select best camera for each object)
        objects_by_camera = {}
        for obj in frame.objects:
            if not obj.visible_cameras:
                continue
            
            # Choose best camera for this object based on preference and visibility
            best_camera = self._select_best_camera(obj, frame, camera_preference)
        ...
        # Load all camera images needed
        camera_images = {}
        for camera in objects_by_camera.keys():
            camera_token = frame.camera_tokens.get(camera)
            if camera_token:
                image = self._load_camera_image(camera_token)
                if image is not None:
                    camera_images[camera] = image
        
        # Prepare tasks for parallel execution
        annotation_tasks = []
        for camera, camera_objects in objects_by_camera.items():
            if camera not in camera_images:
                continue
            
            image = camera_images[camera]
            
            # Submit annotation task for each object
            for obj in camera_objects:
                future = self.executor.submit(
                    self._annotate_single_object,
                    obj, image, camera, camera_objects, frame
                )
                annotation_tasks.append(future)
        
        # Collect results
        annotated_objects = []
        for future in tqdm(annotation_tasks, desc="  Annotating objects", leave=False):
            result = future.result()
            if result is not None:
                annotated_objects.append(result)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Annotate nuScenes scene graph with VLM (Parallelized)')
    ...
    parser.add_argument('--max-workers', type=int, default=10, help='Max parallel workers for API calls')
    parser.add_argument('--camera-preference', nargs='+', default=['CAM_FRONT', 'CAM_FRONT_LEFT', 'CAM_FRONT_RIGHT'],
                        help='Ordered list of preferred cameras')
    ...
    print(f"Initializing VLM annotator (max_workers={args.max_workers})...")
    ...
    try:
        ...
        results = annotator.annotate_scene(
            ...
            camera_preference=args.camera_preference
        )
